...
class Encoder(nn.Module):
    ...
    def forward(self, input):
        ...
        if self.model_type == 'LSTM':
            ...
            hidden_transformed = self.fc2(self.relu(self.fc1(hidden[-1])))
            hidden_transformed = self.tanh(hidden_transformed)
            hidden_transformed = hidden_transformed.unsqueeze(0)
            hidden_new = (hidden_transformed, cell)
            return output, hidden_new
        else:
            output, hidden = self.rnn(embedded)
            hidden = self.fc2(self.relu(self.fc1(hidden)))
